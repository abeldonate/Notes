...
\begin{proof}
\com{insertar diagrama}

The first thing we notice is that there exist a functor $\Gamma_P$ that sends the sections to the stalks and the morphisms of sections to the morphisms of staks. This functor is exact \com{Is this true, guess so, and right is true}

To prove the injectivity of $\varphi |_U$ is immediate. It remains to prove that $ \ker (\psi |_U) = \im (\varphi |_U )$. The inclusion $\boxed{ \supseteq }$ cames from the functoriality, meaning $\psi |_U \circ \varphi |_U = \Gamma (\psi   ) \circ   \Gamma (\varphi  ) = \Gamma (\psi  \circ  \varphi ) = 0$. 
